...
# ### Load the facility reference table:
#
# `ref_facilities` is generated by `01a_build_facility_topology` from `TOPOLOGY_SEED`. It
# used to be built here, either by scraping EPA Envirofacts or -- whenever that call failed,
# which was most of the time -- by laying a 0.5 degree grid of `PB_nnn` points across the
# BBOX and attributing plumes to grid squares.
#
...
permian_fac = spark.table("ref_facilities").toPandas()

required = ["facility_id", "facility_name", "facility_lat", "facility_lon"]
missing = [c for c in required if c not in permian_fac.columns]
assert not missing, (
    f"ref_facilities is missing {missing}. Run 01a_build_facility_topology to rebuild it."
)
assert len(permian_fac) > 0, "ref_facilities is empty -- run 01a_build_facility_topology first"

# The PB_nnn grid fallback used to land here whenever the EPA call failed. Grid points are
# not facilities, and attributing a plume to one produced a site id that meant nothing.
grid = permian_fac["facility_id"].astype(str).str.match(r"^PB_\d+$")
assert not grid.any(), (
    f"{int(grid.sum())} PB_nnn grid reference points found in ref_facilities -- the table was "
    "overwritten by the retired fallback. Rerun 01a_build_facility_topology."
)

print(f"Loaded {len(permian_fac)} facilities from ref_facilities")
print(f"Columns: {list(permian_fac.columns)}")

if "region_band" in permian_fac.columns:
    print("\nBand distribution:")
    for band, n in permian_fac["region_band"].value_counts().items():
        print(f"  {band:<12}{n:>5}")

if "in_detection_bbox" in permian_fac.columns:
    n_out = int((~permian_fac["in_detection_bbox"]).sum())
    print(f"\n{n_out} facilities sit outside the detection BBOX. They stay in the candidate")
    print("set on purpose, so a plume near an edge can still attribute across it.")

print()
print(permian_fac[required].head(10).to_string(index=False))
